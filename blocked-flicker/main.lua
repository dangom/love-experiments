...
function love.load(arg)
    ...
    task.ON_BLOCKSIZE = tonumber(arg[3]) or 5 -- s
    task.OFF_BLOCKSIZE = tonumber(arg[4]) or 10 -- s
    ...
    -- Task timing
    task.timing = {}
    task.timing.OFFSET = tonumber(arg[6]) or 2
    task.timing.TOTAL_DURATION = tonumber(arg[7]) or 20
    ...
    task.FLICKER_FREQUENCY = tonumber(arg[8]) or 12 -- flickering in Hz.
    ...
    print(string.format(
              "Setup: BLOCK_ON=%.2f Hz, BLOCK_OFF=%d, Luminance=%.2f %%, Flicker=%d Hz",
              task.ON_BLOCKSIZE, task.OFF_BLOCKSIZE, 100 * task.LUMINANCE,
    ...
    task.IS_SCALEDNOISE = toboolean(arg[9]) or false
    ...
    state.modulation_time = -task.timing.OFFSET -- this keeps track of how much the oscillation has evolved
...
function love.update(dt)
    ...
    -- Advance block time
    state.modulation_time = state.modulation_time + dt
    if state.modulation_time > (task.ON_BLOCKSIZE + task.OFF_BLOCKSIZE) then
        state.modulation_time = state.modulation_time -
                                    (task.ON_BLOCKSIZE + task.OFF_BLOCKSIZE)
    end
...
local instructions = love.graphics.newImage("assets/instructions_ready.png")
local function draw_instructions()
    -- This draws a splashscreen before the arrival of the first trigger.
    -- love.graphics.setColor(colors["FOREGROUND"])
    love.graphics.setBackgroundColor(1, 1, 1)
    love.graphics.setColor(255, 255, 255, 255)
    -- love.graphics.setFont(love.graphics.newFont(64))
    -- love.graphics.printf("The task will begin shortly...", 0,
    --                      window.HEIGHT / 2 - 32, window.WIDTH, 'center')
    -- This draws the instructions centered on the screen. Scale is set to 0.75.
    love.graphics.draw(instructions, love.graphics.getWidth() / 2,
                       love.graphics.getHeight() / 2, 0, 0.75, 0.75,
                       instructions:getWidth() / 2, instructions:getHeight() / 2)
end

local function draw_await()
        love.graphics.setBackgroundColor(0.5, 0.5, 0.5)
...
function love.draw()
    ...
    if not state.is_running then
        if not state.task_understood then
            draw_instructions()
        else
            draw_await()
        end
    ...
    if not state.is_finished then
        ...
        if state.time < task.timing.OFFSET then
            ...
            state.alpha = state.modulation_time <= task.ON_BLOCKSIZE and
                              task.LUMINANCE or 0
        end
...
function love.keypressed(key, scancode)
    ...
    if not task.is_running then
        state.task_understood = true
    end

    if key == "escape" then
        if #reactions > 0 and #reaction_times > 0 then
            results.hitrate = 100 * mathutils.sum(reactions) / #reactions
            results.avg_rt = mathutils.sum(reaction_times) / #reaction_times
        else
            results.hitrate = 0
        ...
        events[#events + 1] = {
            state.time, 0, state.trigger_count, "CANCELLED", "n/a", string.format("%.2f %%", results.hitrate)
        }
        save_data(events, task)
    ...
    -- Received keypress
    if key == "5" or key == "6" or key == "7" or key == "8" or key == "9" or key == "1" or key == "2" or key == "3" or key == "4" or key == "0" then
